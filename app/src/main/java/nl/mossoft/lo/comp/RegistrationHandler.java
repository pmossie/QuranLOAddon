...
public final class RegistrationHandler {
  ...
  // Registration guard to prevent duplicate work
  private static final AtomicBoolean registrationCompleted = new AtomicBoolean(false);
  private static final AtomicInteger registrationAttempts = new AtomicInteger(0);
  ...
  public static boolean __writeRegistryServiceInfo(final XRegistryKey registryKey) {
    ...
    LOGGER.debug(
        "Registration attempt #{} on thread: {}", attempt, Thread.currentThread().getName());

    if (registrationCompleted.get()) {
      LOGGER.debug("Registration already completed, skipping duplicate call");
      return true; // Return true since registration already succeeded
    }
    ...
    if (result[0]) {
      ...
      LOGGER.info(
          "Successfully registered {} service classes (attempt #{})",
          SERVICE_CLASSES.size(),
          attempt);
    } else {
      LOGGER.error("Failed to register some service classes (attempt #{})", attempt);
    }
